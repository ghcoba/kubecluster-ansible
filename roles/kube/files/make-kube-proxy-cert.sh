...
tmp_cert_config_dir="${TMP_CERT_CONFIG_DIR:-"/etc/tmp_kube_proxy_certs_config"}"
...
cn_name_current="system:kube-proxy"
o_name_use="system:kube-proxy"
...
if ! (./cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=kube kube-proxy.json | ./cfssljson -bare kube-proxy) >/dev/null 2>&1; then
